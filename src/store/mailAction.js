...
export const fetchDataWithTimer = () => {
    ...
    return async (dispatch, getState) => {
        ...
        const poll = async () => {
            try {
                ...
                const {inbox,sentbox} = await fetchDataWithTimer();
                const newIds = inbox.map(ele=>ele.id);
                const changed = newIds.length!==lastIndexs.length || newIds.some((id,idx)=>id!==lastIndexs[idx]);
                if(changed){
                    lastIndexs = newIds
                    dispatch(mailActions.fetchMessages({inbox,sentbox}));
                }
            } catch (error) {
